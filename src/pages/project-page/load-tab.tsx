...
const LoadTab = ({files, categories}: LoadTabProps) => {
    ...
    return (
        <div className={"load-tab"}>
            <CardCmp className={"load-tab__categories-container"}>
                {
            ...
            <CardCmp className={"load-tab__upload-container"}>
...
const CategoryFiles = ({name, files}: CategoryFilesProps) => {
    ...
    return (
        <div className={`category-files ${isOpen ? "category-files_open" : ""} ${files?.length ? "" : "category-files_empty"}`}>
